...
test('more than was asked for still passes', () => {
  const ask = { slide: 'boardslide' as const, label: 'boardslide' }
  // A plain one.
...
test('a name would have got these wrong, and the parts do not', () => {
  const ask = { flips: 1, flipSign: 1, label: 'kickflip' }
...
test('what is not asked for is not looked at', () => {
  const loose = { slide: 'boardslide' as const, label: 'boardslide' }
  const strict = { slide: 'boardslide' as const, side: 'frontside' as const, label: 'fs board' }
